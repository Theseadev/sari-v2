...
function getCatalogStyles(): string {
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Intercept "Baca Sekarang" / "Lanjut" links inside modal
            document.body.addEventListener('click', (e) => {
                const readLink = e.target.closest('a[href^="/baca/"]');
                if (readLink && document.getElementById('bookModal')) {
                    e.preventDefault();
                    sessionStorage.setItem('returnTo', window.location.href);
                    window.location.href = readLink.getAttribute('href');
                }
            });
            ...
            // If returning from reader, restore the modal
            const returnTo = sessionStorage.getItem('returnTo');
            if (returnTo) {
                sessionStorage.removeItem('returnTo');
                const url = new URL(returnTo);
                const slug = url.pathname.split('/buku/')[1];
                if (slug) setTimeout(() => openModalWithSlug(slug), 100);
            }

            // If URL has ?open=slug (from redirect), open modal
            const openParam = new URLSearchParams(window.location.search).get('open');
            if (openParam) {
                window.history.replaceState({}, '', '/buku');
                setTimeout(() => openModalWithSlug(openParam), 100);
            }
        });
    </script>
    `;
...
export async function detail(c: Context) {
    ...
    // Selalu kembalikan Partial Modal ( AJAX )
    // Direct URL access → redirect ke katalog, modal dibuka via JS
    if (!isAjax) {
        return c.redirect(`/buku?open=${esc(current.slug)}`);
    }
